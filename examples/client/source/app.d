...
void main() {
	...
	client.run( Command("fullscreen", "toggle") );
	...
	foreach( node ; client.getTree() ) {
		writeln( node );
	}


	// Subscribe a Lambda function to given Events
	client.subscribe( [EventType.input, EventType.window] , (msg) => writefln("%s-event has been triggered",msg.type) );
