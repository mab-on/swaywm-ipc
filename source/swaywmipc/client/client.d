...
import swaywmipc.client.command;
...
class Client {
	...
	Response sendCommand(Command cmd) { return this.sendCommand( [cmd] ); }
	...
	Response subscribe(Event[] events , void function(Message msg) fn) {
		...
		Message reply;
		Message subscribe = events.NewSubscribeMessage();
	...
	void stopEventListener() {
		static import std.socket;
		send(this.event_listener, CancelListener());
